...
const Template = ({ children, className, defaultClassName, icon, ...args }: TemplateProps) => {
  return (
    <button
      type="button"
      className={`${className ? `${className} ` : ''}${defaultClassName} effect-pressing`}
      {...args}
    >
      {icon && (
...
const Button = ({ type = 'default', icon, className, children = '', ...rest }: ButtonProps) => {
  switch (type) {
    case 'menu-default':
      return (
        <Template
