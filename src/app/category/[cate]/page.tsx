...
import { getAllCategory } from '#/services/post'

export const generateStaticParams = async () => {
  const categories = await getAllCategory()
  console.log('categories', categories);
  const paths = categories.map((cate) => ({
    category: encodeURI(cate.slug),
  }))
  return paths
}
...
const CatePage = async (props: { params: Promise<{ cate: string }> }) => {
  ...
  const categories = await getAllCategory()
  const category = categories.find((cate) => cate.slug === info.cate)
    ...
    return (
      <>
        <Head>
          ...
          />
          <meta name="description" content={`TonyHe's content under category "${category.name}"`} />
        </Head>
          ...
          <div className="mb-4 items-center lg:flex">
            <div className="flex-1 items-center">
              <h1 className="flex justify-center text-1 font-medium tracking-wide text-black dark:text-white lg:justify-start">
                ...
                <span data-cy="cateName">{category.name}</span>
              </h1>
            </div>
        ...
        <div className="mt-10 lg:mt-5">
          <InfiniteList url={`/api/posts?category=${category.slug}`} />
        </div>
      </>
    )
  } else {
