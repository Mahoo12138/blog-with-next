...
const Tabs = (props: TabsProps) => {
  ...
  const styleHighlighter = (
    ...
    if (visible) {
      highlighterRef.current.style.width =
        highlightedIndex === -1 && defaultHighlighted && direction === 'vertical'
          ? '100%'
  ...
  const highlight = (
    ...
    if (verticalListWrapper) {
      scrollToItemWithinDiv(verticalListWrapper.current, listRef.current.children[index])
    }

    const targetTabBoundingBox =
      e instanceof Element ? e.getBoundingClientRect() : e.currentTarget.getBoundingClientRect()
    const wrapperBoundingBox = wrapperRef.current?.getBoundingClientRect()
    styleHighlighter(true, wrapperBoundingBox, targetTabBoundingBox, bgColor, bgDark, className)
  ...
  if (direction === 'vertical') {
    ...
    useHotkeys(
      ...
      (e) => {
        e.preventDefault()
        const targetIndex = highlightedIndex + 1 < items.length ? highlightedIndex + 1 : 0
        highlight(
          listRef.current.children[targetIndex],
          items[targetIndex].bgColor,
          items[targetIndex].bgDark,
    ...
    useHotkeys(
      ...
      (e) => {
        e.preventDefault()
        const targetIndex = highlightedIndex - 1 >= 0 ? highlightedIndex - 1 : items.length - 1
        highlight(
          listRef.current.children[targetIndex],
          items[targetIndex].bgColor,
          items[targetIndex].bgDark,
    ...
    >
      ...
      <ul
        ...
        className={`list-none items-center ${
          direction === 'vertical' ? 'grid grid-flow-row' : 'flex flex-row gap-x-2'
        }`}
        ref={listRef}
      >
