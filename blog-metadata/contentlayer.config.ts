...
import { Blog as Post } from '@blog/metadata/post'
...
function createTagCount(allBlogs: Post[]) {
  ...
  writeFileSync(path.resolve(root, './data/tag-data.json'), JSON.stringify(tagCount))
}
...
function createCateCount(allBlogs: Post[]) {
  ...
  writeFileSync(path.resolve(root, './data/cate-data.json'), JSON.stringify(cateCount))
}
...
export default makeSource({
  contentDirPath: root,
  contentDirExclude: ['.contentlayer', 'data/*.json', 'node_modules'],
  documentTypes: [Blog],
  mdx: {
    cwd: process.cwd(),
    remarkPlugins: [
